function dy=carriersoln31(t,y,ie,vcb,ld)
...
jt=(power((2*me),1/2)*(q^3)*ep*v*(1/(4*(pi^3)*(hb1^2)*eg^0.5)))*exp((-4*power((2*me),1/2)*eg^1.5)/(3*q*ep*hb1));

nw=(0.8*ld*(d/dn)*(0.4/eps)*vg*afk*y(3))+(ld/(q*dn))*abs(jt);
...
%jc=(q*(dn/ld)*(1/sinh(dbw/(2*ld))))*(y(1)*power(cosh(dbw/(2*ld)),2)+(nw*cosh(dbw/(2*ld))));
