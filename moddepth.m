...
for i4=1:length(t1)
  i2(j3,i4)=ie+(1e-3)*sin(2*pi*f*t1(i4));%+(1.5e-3)*sin(2*pi*f1*t1(i4));%Input Emitter Current
end
j=1;j1=1;
for i3=1:length(i2(j3,:))
if j==1
    [ti yi ]=ode45(@carriersoln31,tspan,[0;0;0],options,i2(j3,i3),vcb,ld);
[t y ]=ode45(@carriersoln31,tspan,[yi(length(yi),1);yi(length(yi),2);yi(length(yi),3)],options,i2(j3,i3),vcb,ld);
else
